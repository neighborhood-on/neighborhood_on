...
// Next.js 전용 라우팅 모듈 대신, 브라우저 API를 사용하도록 수정합니다.
// import { useRouter, useSearchParams } from 'next/navigation'; // 이 부분 제거
...
/**
 * Next.js useSearchParams 대신 브라우저의 URL 쿼리 파라미터를 읽는 커스텀 훅.
 */
...
// =================================================================
// 3. CORE LOGIC (커뮤니티 보드 컴포넌트)
// =================================================================

const CommunityBoard = ({ neighborhoodId }: { neighborhoodId: string }) => {
  const [selectedCategory, setSelectedCategory] = useState<string>('전체');
  const [searchQuery, setSearchQuery] = useState<string>('');
  
  // 현재 동네 이름 (ID를 기반으로 간단히 매핑)
  const neighborhoodName: string = neighborhoodId.includes('gangnam') ? '강남구' : neighborhoodId.includes('songpa') ? '송파구' : '알 수 없는 동네';

  // 1. 동네별 필터링
  const neighborhoodPosts = useMemo<Post[]>(() => {
    return mockPosts.filter(post => post.neighborhoodId === neighborhoodId);
  }, [neighborhoodId]);
  ...
  // 게시글 클릭 핸들러 (실제로는 상세 페이지로 이동)
  const handlePostClick = (postId: number) => {
    // Next.js 라우터 대신 브라우저의 window.location.href 사용
    window.location.href = `/post?id=${postId}&neighborhood=${neighborhoodId}`;
    console.log(`${neighborhoodId}의 ${postId}번 게시글 상세 페이지로 이동합니다.`);
  };

  // Back 버튼 핸들러 (지도 페이지로 이동)
  const handleBackToMap = () => {
    // router.push('/map'); 대신 window.location.href 사용
  ...
  return (
    <div className="community-board-wrapper">
      ...
      <footer className="board-footer">
        <p>현재 {neighborhoodName} 게시판이 표시되고 있습니다.</p>
      </footer>
    </div>
  );
};
...
// =================================================================
// 4. NEXT.JS PAGE COMPONENT (실제 페이지 진입점)
// =================================================================

// board/page.tsx는 이 함수를 export 합니다.
const BoardPage = () => {
  const [isClient, setIsClient] = useState(false);
  
  // 클라이언트 측에서만 URL을 읽기 위해 useEffect를 사용하여 상태를 업데이트합니다.
  useEffect(() => {
    setIsClient(true);
  }, []);
    
  const { get: getParam } = useClientSearchParams();
  const neighborhoodId = getParam('id'); // URL 쿼리 파라미터에서 'id'를 추출합니다.

  // 클라이언트 환경이 준비되지 않았거나 ID가 없는 경우 로딩 또는 에러 메시지를 표시
  if (!isClient) {
    return (
      <div className="app-container">

        <div className="app-content-wrapper p-10 text-center text-gray-500">
  ...
  // 쿼리 파라미터가 없는 경우 에러 메시지 또는 기본값 처리
  if (!neighborhoodId) {
    return (
      <div className="app-container">
        <div className="app-content-wrapper">
          <div className="error-card">
            <h1 className="text-2xl font-bold mb-4">⚠️ 동네 정보를 불러올 수 없습니다.</h1>
            <p className="text-gray-700">URL 쿼리 파라미터(id)를 통해 접속할 동네 ID를 지정해주세요. Map 페이지에서 동네를 선택해주세요.</p>
            <button
                onClick={() => window.location.href = '/map'} // window.location을 사용해 간편하게 Map 페이지로 리디렉션
  ...
  return (
    <div className="app-container">
      <div className="app-content-wrapper">
        <CommunityBoard neighborhoodId={neighborhoodId} />
      </div>
    </div>
  );
};
